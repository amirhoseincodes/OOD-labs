...
| ---- | -------------------------------- | ---------------- | ------------------------------- | ------------------------------------ | -------------------- | ------------------------------------------------------- |
| 1    | افزودن تابع `sendSmsMessage`     | `MessageSender`  | اضافه‌کردن روش جدید ارسال پیامک | افزودن `INPERSON` به enum            | `PaymentMethods`     | اضافه‌کردن گزینه پرداخت حضوری                           |
| 2    |                                  |                  |                                 | افزودن متود `payInPerson`            | `PaymentProcessor`   | متود جدید برای چاپ کردن روش پرداخت                      |
| 3    |                                  |                  |                                 | اضافه کردن `INPERSON` به switch case | `ReservationService` | این قسمت `payment processor` را از قسمت قبلی صدا می زند |
